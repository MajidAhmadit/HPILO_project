...
#    echo -n $Host >> $LogFile
    Details=$(perl locfg.pl -s ${Host}-sc.ott.ciena.com -f scripts/gethealth.xml |grep -A 8 "<HEALTH_AT_A_GLANCE>"| grep -v HEALTH_AT_A_GLANCE|grep -v \"OK\"|grep -v "BATTERY STATUS= \"Not Installed\"")
