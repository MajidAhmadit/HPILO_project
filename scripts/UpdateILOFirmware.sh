...
for Host in $List; do
    ...
    Product_Name=$(perl locfg.pl -s ${Host}-sc.ott.ciena.com -f ILOXMLFile/Get_Host_Data.xml |grep "Product Name"|awk -F "\"" '{print $4}')
...
case $Product_Name in
	...
	ProLiant*BL*G*8* | ProLiant*DL*G*8* | ProLiant*G*9*|ProLiant*WS*G*8*)
		...
		check_version
	        if [ $flag -eq 1 ] 
			then
			perl locfg.pl -s ${Host}-sc.ott.ciena.com -f ILOXMLFile/Update_Firmware_ilo4.xml
		fi

        	;;
	ProLiant*G7* | ProLiant*BL*G7)
		LastVersion=1.94
		Details=$(perl locfg_v6.pl -s ${Host}-sc.ott.ciena.com -f ILOXMLFile/getquickstats.xml |grep -A 3 "<INDEX_4>"|grep "FIRMWARE_VERSION" | tail -l)	
		CurrentVersion=`echo $Details|awk -F "\"" '{print $2}'|cut -c1-4`
		check_version
                 if [ $flag -eq 1 ] 
	                then
		  	perl locfg.pl -s ${Host}-sc.ott.ciena.com -f ILOXMLFile/Update_Firmware_ilo3.xml 
		 fi
		;;

	ProLiant*G6* | ProLiant*BL*G6)
		LastVersion=2.33
		Details=$(perl locfg.pl -s ${Host}-sc.ott.ciena.com -f ILOXMLFile/getquickstats.xml |grep -A 3 "<INDEX_6>"|grep "FIRMWARE_VERSION" | tail -l)
                CurrentVersion=`echo $Details|awk -F " " '{print $4}'|cut -c2-5`
		check_version
                 if [ $flag -eq 1 ] 
			then
                   	perl locfg.pl -s ${Host}-sc.ott.ciena.com -f ILOXMLFile/Update_Firmware_ilo2.xml 
 		 fi 
	        ;;
	...
	ProLiant*DL*G*10* )
		LastVersion=2.44
                ...
                if [ $flag -eq 1 ]
			then
                        perl locfg.pl -s ${Host}-sc.ott.ciena.com -f ILOXMLFile/Update_Firmware_ilo5.xml
                fi
